...
static constexpr int ULTRA_PERIOD_MS = 40; //actually 80 per sensor, each function run only updates one of the sensors
...
static constexpr int FOLLOW_LOST_CONFIRM_MS = 20;
static constexpr int FOLLOW_LOST_TICKS      = FOLLOW_LOST_CONFIRM_MS / CTRL_PERIOD_MS;

static constexpr int ALIGN_LOST_CONFIRM_MS = 20;
...
static constexpr int   TURN_45_MS            = 600;
static constexpr float DUTY_OB_TURN          = 0.71f;
static constexpr float DUTY_OB_FWD           = 0.40f;
static constexpr float DUTY_OB_RIGHT         = 0.71f;
...
// Color sensor
tcs3472::TCS3472 color(PTE0, PTE1, 100000); // SDA=PTE0, SCL=PTE1
...
static bool ultra_read_front_next = true;
...
enum class ObPhase { TURN_LEFT_45, SEARCH_RIGHT_WALL, REACQUIRE }; //Obstacle-avoid sub states
...
static void controller_update() {
    ...
    if (state != rt.prev_state) {
        if (state == CtrlState::OBSTACLE_AVOID) rt.ob_reset_pending = true;
        rt.prev_state = state;
    }

    // If obstacle mode was latched, force it
    if (rt.ob_active && state != CtrlState::OBSTACLE_AVOID) {
    ...
    if (!lock_ob) {
        ...
        if (state != CtrlState::FULLY_STOPPED &&
            ...
            red_confirmed &&
            sh.right_cm < Config::FRONT_MAX_CM) // ensure the traffic light is actually there to avoid red lockers doing stupid things
        {
            rt.red_ctrl_cnt = 0;
    ...
    switch (state) {
        ...
        case CtrlState::OBSTACLE_AVOID: {
            ...
            if (rt.ob_reset_pending) {
                ...
                rt.phase_until      = {};
                rt.line_found_cnt   = 0;
                rt.prev_right_valid = false;
                rt.prev_right_cm    = 0.0f;
                rt.right_inc_cnt    = 0;
                rt.sub_brake_until  = {};
            ...
            auto set_phase = [&](ObPhase p) {
                if (rt.phase != p) {
                    rt.phase = p;
                    rt.sub_brake_until = Kernel::Clock::now() + 50ms;
                    leds_for_ob_phase(rt.phase);
                }
            };

            // Line exit condition -> debounced tho
            if (!li.lost) rt.line_found_cnt++;
            else          rt.line_found_cnt = 0;

            if (rt.did_follow && rt.line_found_cnt >= 3) {
                rt.line_found_cnt = 0;
                rt.ob_active = false;
            ...
            // “increasing” vs previous sample
            bool right_increasing_now = false;
            if (right_seen) {
                if (rt.prev_right_valid) right_increasing_now = (sh.right_cm > rt.prev_right_cm + 2.0f);
                rt.prev_right_cm    = sh.right_cm;
                rt.prev_right_valid = true;
            } else {
                rt.prev_right_valid = false;
                rt.right_inc_cnt    = 0;
            }

            //used to check whether we are moving away
            const bool right_increasing = debounce(right_seen && right_increasing_now, rt.right_inc_cnt, 2);
            ...
            switch (rt.phase) {
                ...
                case ObPhase::REACQUIRE: {
                    ...
                    if (right_increasing) set_phase(ObPhase::SEARCH_RIGHT_WALL);
                    break;
                }
            }
...
int main() {
    ...
    // Start behavior (your comment says fully stopped, but your code did SEEK_LEFT; keeping your code)
    enter_state(CtrlState::SEEK_LEFT, 0);
