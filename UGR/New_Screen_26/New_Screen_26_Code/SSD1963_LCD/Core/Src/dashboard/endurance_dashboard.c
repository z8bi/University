...
static EnduranceDashboardLayout g_layout;

static void endurance_build_layout(EnduranceDashboardLayout *L,
                                   const DashboardTheme *T)
{
    ...
    L->lap_block.area = make_rect(L->right_section.x, right_blocks_y0,
                                  L->right_section.w, right_block_h);
...
static void endurance_draw_static(const EnduranceDashboardLayout *L)
{
    SSD1963_Fill(COL_BG);
...
static void endurance_draw_values_full(const EnduranceDashboardLayout *L,
                                       int battery_charge,
...
void endurance_dash_init(double initial_battery_charge,
                         ...
                         int initial_speed)
{
    endurance_build_layout(&g_layout, &g_theme);
    ...
    endurance_draw_static(&g_layout);
    endurance_draw_values_full(&g_layout,
                               prev.battery_charge,
...
void endurance_dash_update(const Dashboard *d)
{
    ...
    if (!inited) {
        endurance_dash_init(d->battery_charge,
                            d->cell_temperature,
                            d->lap,
                            d->speed);
        return;
